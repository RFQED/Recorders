...
import ResistanceUI # This file holds our MainWindow and all design related things
              # it also keeps events etc that we defined in Qt Designer

class ResistanceApp(QtGui.QMainWindow, ResistanceUI.Ui_MainWindow):
    def __init__(self):
        super(self.__class__, self).__init__()
        self.setupUi(self)  # This is defined in ResistanceUI.py file automatically
        ...
        #self.email_box.setPlaceholderText(placeHolder)
...
###############################################################################################################
    ...
    def ELOG(self):
        ...
        os.environ["RESISTANCE_ELOG_COMMENT"]= "Resistances for Module " + str(ModuleNumber) + " were recorded on " + Localtime
        os.environ["RESISTANCE_FILENAME"] = str("Module_" + str(ModuleNumber) + "_ResistanceData_"+str(date)+".txt")

#        subprocess.call("./elog_script.sh", shell=True)
        subprocess.call("./resistance_elog_script.sh", shell=True)
        print "sent to elog"
...
################################################################################################################
    ...
    def SaveData(self):
        ...
        filename = "Module_" + str(ModuleNumber) + "_ResistanceData_"+str(date)+".txt"
        ...
        f = open(filename, 'w')
        f.write(
            "Resistance Data for Module Number " + str(ModuleNumber) + " Recorded on " + Localtime + " measured in gramms \n"
...
######################################################################
    def email(self):
        if (self.email_box.text() != ""):
            ...
            ModuleNumber = int(self.ModuleNumber.text())
            date = time.strftime("%d_%m_%y")
            SUBJECT = "Resistance Data from Module " + str(ModuleNumber)
            filename = "Module_" + str(ModuleNumber) + "_ResistanceData_"+str(date)+".txt"
...
def main():
    ...
    form = ResistanceApp()                 # We set the form to be our ExampleApp (design)
